...
export default function Dashboard() {
    ...
    const handleReturn = async (bookId) => {
        if (!user) return;
        await returnBook(bookId, user.userId); // Pass the logged-in user's ID
    ...
    useEffect(() => {
        if (user?.userId) {
            axiosClient
                ...
                .then(res => setBorrowedBooks(res.data.filter(b => b.returnedAt === null)))
                .catch(err => console.error('Failed to fetch borrowings', err));
        }
    }, [user]);
                ...
                {borrowedBooks.length === 0 ? (
                    <div className="bg-white p-6 rounded-xl shadow-[0_2px_6px_rgba(0,0,0,0.05)] border border-[#eee]">
                        <p className="text-[#555]">
                            Zatím nemáte žádné aktivní výpůjčky.
                        </p>
                    </div>
                ) : (
                    borrowedBooks.map((borrowing) => {
                        if (!borrowing.book) return null;
                        ...
                        return (
                            ...
                            >
                                <div className="flex w-full sm:w-auto items-center mb-4 sm:mb-0">
                                    ...
                                    <div>
                                        <h3 className="m-0 mb-1 text-[#222] text-xl font-bold">
                                            {borrowing.book.title}
                                        </h3>
                                        <p className="text-[#555] text-sm m-0">
                                            Autor: {borrowing.book.author}
                                        </p>
                                        <p className="text-[#555] text-sm m-0 mt-1">
                                            Vypůjčeno:{' '}
                                            {new Date(borrowing.borrowedAt).toLocaleDateString('cs-CZ')}
                                        </p>
                                        <p className="text-[#555] text-sm m-0 mt-1">
                                            Vrátit do:{' '}
                                            {new Date(borrowing.dueDate).toLocaleDateString('cs-CZ')}
                                        </p>
                                    </div>
                                </div>

                                <button
                                    onClick={() => handleReturn(borrowing.book.bookId)}
